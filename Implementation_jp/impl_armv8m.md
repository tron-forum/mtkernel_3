# μT-Kernel3.0 ARMv8-Mマイコン向け実装仕様書 <!-- omit in toc -->
## Version.02.00.00 <!-- omit in toc -->
## 2024.12.25 <!-- omit in toc -->
...
- [2. 基本実装仕様](#2-基本実装仕様)
  ...
  - [2.2. メモリ関連](#22-メモリ関連)
    ...
    - [2.2.2. スタック](#222-スタック)
    - [2.2.3. 動的メモリ管理](#223-動的メモリ管理)
    - [2.2.4 システムメモリ](#224-システムメモリ)
...
# 1. 概要
## 1.1. 目的
本書はARMv8-Mコアを使用したマイコン向けのμT-Kernel3.0の実装仕様を記す。  
対象は、トロンフォーラムから公開されているμT-Kernel 3.0(V3.00.08)のうち、ARMv8-Mコアを使用したマイコン向けの実装部分である。  
...
## 2.1. マイコン関連
### 2.1.1. 実行モードと保護レベル
ARMv8-Mは、プログラムの動作モードとして、スレッドモードとハンドラモードの二つのモードをもつ。スレッドモードは、特権レベルまたは非特権レベルの実行レベルを割り当てることができる。ハンドラモードは特権レベルに限定される。本実装では、すべてのプログラムは特権レベルで実行することとする。  
また、ARMv8-MはTrustZoneの機能を持ち、プログラムをセキュア、非セキュアに分けることができる。ただし、現在の実装はTrustZoneを使用しない。

### 2.1.2. CPUレジスタ
ARMv8-Mは内部レジスタとして、汎用レジスタ(R0~R12)、SP(R13)、LR(R14)、PC(R15) 、xPSRを有する。

OSのAPI (tk_set_reg、 tk_get_reg) を用いて実行中のタスクのコンテキストのレジスタ値を操作できる。
APIで使用するマイコンのレジスタのセットは 以下のファイルにて定義される。

    include/tk/sysdepend/cpu/core/armv8m/cpudef.h 
...
タスク属性の指定は、タスク生成API(tk_cre_tsk)にてタスク生成時に指定する。または、次項のコンフィギュレーションによっても行うことができる。  

(3) コンフィギュレーションによるTA_FAU属性付加  
コンフィギュレーション(config.h)にてALWAYS_FPU_ATRを有効(1)にすると、全てのタスクがTA_FPU属性となる。この指定は、タスク生成API(tk_cre_tsk)のタスク属性の指定より優先される。
...
## 2.2. メモリ関連
### 2.2.1. メモリモデル
ARMv8-Mは32bitのアドレス空間を有する。MMU(Memory Management Unit：メモリ管理ユニット)は有さず、単一の物理アドレス空間のみである。  
本実装では、プログラムは一つの実行オブジェクトに静的にリンクされていることを前提とする。OSとユーザプログラム（アプリケーションなど）は静的にリンクされており、関数呼び出しが可能とする。  

### 2.2.2. スタック
ARMv8-Mには、MSP(Main Stack Pointer)とPSP(Process Stack Pointer)の二種類のスタックが存在する。ただし、本実装ではMSPのみを使用し、PSPは使用しない。  
...
### 2.2.3. 動的メモリ管理
コンフィギュレーション(config.h)のUSE_IMALLOCが1の場合、動的メモリ管理に使用するシステムメモリが確保される。USE_IMALLOCが0の場合、動的メモリ管理は無効となる。USE_IMALLOCの初期値は1である。  
動的メモリ管理が有効の場合、OSは以下のメモリをシステムメモリ領域から動的に確保する。  

(1) タスクのスタック (但し属性TA_USERBUFのタスクは除く)  
(2) メモリプール領域 (但し属性TA_USERBUFのメモリプールは除く)  
(3) メッセージバッファ領域 (但し属性TA_USERBUFのメッセージバッファは除く)  

### 2.2.4 システムメモリ
システムメモリは動的メモリ管理に使用されるメモリ領域である。動的メモリ管理が有効(コンフィギュレーションのUSE_IMALLOCが1)の場合、以下の規則に従い確保される。  

(1) 静的のシステムメモリ確保  
コンフィギュレーション(config.h)のUSE_STATIC_SYS_MEMが1の場合、システムメモリはグローバル変数として静的に確保される。その際のシステムメモリのサイズは、コンフィギュレーションのSYSTEM_MEM_SIZEで指定される。  

(2) 動的なシステムメモリ確保  
コンフィギュレーション(config.h)のUSE_STATIC_SYS_MEMが0の場合、システムメモリはOS起動時にメモリの空き領域から以下の手順で確保される。  
...
(※)ARMv8-Mの外部割込みの本数は最大480である。ただし、実際の本数はマイコンの実装により異なる。外部割込みの本数は各マイコンの実装において N_INTVEC として定義される。
...
### 2.3.3. ベクタテーブル
ARMv8-Mは、前述の各種例外に対応する例外ハンドラのアドレスを設定したベクタテーブルを有する。リセット時のベクタテーブルはマイコン毎にROM上に定義される。具体的には各マイコンの実装仕様書を参照のこと。  
OSの初期化処理において、ROM上のベクターテーブルはRAM上にコピーされ、以降はそちらが使用される。RAM上のベクタテーブルは、以下のファイルにknl_exctblとして定義される。  

     kernel/sysdepend/cpu/core/armv8m/sys_start.c

ただし、コンフィグレーションのUSE_STATIC_IVTが1に設定されている場合は、ROM上のベクターテーブルが使用され続ける。USE_STATIC_IVTの初期値は0である。  

### 2.3.4. 割込み優先度
ARMv8-Mは、8bit(256段階)の割込み優先度を持ち、この8bitを横取り優先度とサブ優先度に分ける。  
...
本実装では、割込み管理機能はARMv8-Mの外部割込みを対象とし、割込みハンドラの管理を行う。その他の例外については対応しない。  
...
- 割込みハンドラ属性  
ARMv8-Mでは、C言語の関数による割込みハンドラの記述がハードウェアの仕様として可能となっている。そのため、TA_HLNG属性とTA_ASM属性のハンドラで大きな差異はなくなっている。  

TA_HLNG属性の割込みハンドラは、割込みの発生後、OSの高級言語対応ルーチンを経由して呼び出される。  
高級言語対応ルーチンは以下のファイルにknl_hll_inthdr関数として記述されている。  

    kernel/sysdepend/cpu/core/armv8m/interrupt.c
...
- デフォルトハンドラ  
割込みハンドラが未登録の状態で、割込みが発生した場合はデフォルトハンドラが実行される。デフォルトハンドラは、以下のファイルにknl_default_handler関数として実装されている。  

     kernel/sysdepend/cpu/core/armv8m/exc_hdr.c
...
上記のAPIおよび関数は以下のファイルに記述される。  
    ...
    lib/libtk/sysdepend/cpu/core/armv8m/int_armv8m.c


- 割込みコントローラ制御  
ARMv8-Mの標準割込みコントローラNVICの制御を行う。なお、各マイコンはNVICの他にも割込み制御機能を有する場合がある。よって、割込みコントローラ制御については、各マイコンの実装仕様書を参照のこと。  
...
NVICの制御は以下のファイルに記述される。
    ...
    lib/libtk/sysdepend/cpu/core/armv8m/int_armv8m.c
    lib/libtk/sysdepend/cpu/core/armv8m/int_armv8m.h
...
これらの例外には、暫定的な例外ハンドラを定義している。暫定的な例外ハンドラは、kernel/sysdepend/cpu/core/armv8m/exc_hdr.cの以下の関数として実装されている。  

| 例外番号 | 例外の種別                | 関数名             |
| -------- | ------------------------- | ------------------ |
| 2        | NMI(ノンマスカブル割込み) | knl_nmi_handler        |
| 3        | ハードフォルト            | knl_hardfault_handler  |
| 4        | メモリ管理                | knl_memmanage_handler  |
| 5        | バスフォールト            | knl_busfault_handler   |
| 6        | 用法フォールト            | knl_usagefault_handler |
| 11       | SVC(スーパーバイザコール) | knl_svcall_handler     |
| 12       | デバッグモニタ            | knl_debugmon_handler   |
...
リセットハンドラは、以下のファイルにReset_Handler関数として実装される。  
     ...
     kernel/sysdepend/cpu/core/armv8m/reset_hdl.c
...
(4) OS起動処理の実行  
OS起動処理関数knl_start_mtkernelを実行する。以降、OSが起動され、リセットハンドラに戻ってくることはない。

リセットハンドラは、コンフィギュレーションのDONT_USE_RESETHDRを1とすることにより無効とすることができる。これはシステムに他にリセットハンドラが存在する場合に指定する。既存の他のファームウェア上でμT-Kernelを実行するときなどを想定している。  

### 3.1.2. OS起動処理
OS起動処理は、以下のファイルにknl_start_mtkernel関数として実装される。  

     kernel/sysdepend/cpu/core/armv8m/sys_start.c

knl_start_mtkernel関数は通常はリセットハンドラから呼び出される。ただし、コンフィギュレーションのDONT_USE_RESETHDRが1の場合は、標準のリセットハンドラは無効となっているので、本関数をリセット処理後に呼び出すようにしなければならない。  
knl_start_mtkernel関数の処理手順を以下に示す。  
...
(7) OS初期化処理(main)の実行  
リセット処理を終了するOSの初期化処理(main)を実行する。本関数に戻ってくることはない。


### 3.1.2. OS初期化処理
OS初期化処理は共通部のmain関数で実行される。共通実装仕様書を参照のこと。  
OS初期化処理のmain関数は、コンフィギュレーションのADD_PREFIX_MAIN_FUNCを1とすることにより、関数名をknl_main関数に変更することができる。これはシステムに他にmain関数が存在する場合に指定する。既存の他のファームウェア上でμT-Kernelを実行するときなどを想定している。  
main関数から以下のARMv8-Mコアに依存する処理が実行される。  

(1) 割込み初期化 (knl_init_interrupt)  
OSが使用する割込みの初期化を行う。 
以下のファイルにknl_init_interrupt関数として定義される。ただし、本実装ではデフォルトの例外ハンドラの登録のみを行っている。    
ユーザは必要に応じて処理を追加することは出来る。   
...
対象ハードウェアに依存する処理のうち、ARMv8-Mコアに共通の処理を記す。各マイコンに依存する処理は、各マイコン向けの実装仕様書を参照のこと。  

(1) システムタイマ終了処理 (knl_terminate_hw_timer)  
システムタイマのハードウェアに依存した終了化を行う。以下のファイルにknl_terminate_hw_timerインライン関数として定義される。  

     kernel/sysdepend/cpu/core/armv8m/sys_timer.h
...
# 5. タスク
ARMv8-Mに依存するタスクの仕様を以下に記す。各マイコン向け実装仕様書も参照のこと。
...
# 6. 時間管理機能
時間管理機能のARMv8-Mコアに依存する仕様を以下に記す。各マイコン向け実装仕様書も参照のこと。
...
| 版数    | 日付       | 内容                                                    |
| ------- | ---------- | ------------------------------------------------------- |
| 2.00.00 | 2024.12.25 | 新規作成(ARMv7-Mマイコン向け実装仕様書をベースとした)<br>版数は他の仕様書にあわせて2.00.00からとする |
