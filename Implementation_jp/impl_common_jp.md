...
## Version.02.01.B0 <!-- omit in toc -->
## 2026.07.08 <!-- omit in toc -->
Copyright © 2019-2026 TRON Forum. All rights reserved.
...
- [3. システムの起動](#3-システムの起動)
  - [3.1. 起動処理](#31-起動処理)
    - [3.1.1. 起動処理の手順](#311-起動処理の手順)
    - [3.1.2. リセットハンドラ](#312-リセットハンドラ)
    - [3.1.3. カーネル起動処理](#313-カーネル起動処理)
    - [3.1.4. OS初期化処理](#314-os初期化処理)
  - [3.2. 初期タスク](#32-初期タスク)
...
対象は、トロンフォーラムから公開されているμT-Kernel 3.0 (V3.00.08)のソースコードの実装とする。
...
| カーネルバージョン番号 | prver   | 0x0008 | リリースバージョン     |
| 製品管理情報           | prno[4] | -      |                        |

### 1.2.3. 対象ハードウェア
本書に記載するμT-Kernel3.0の実装の共通仕様は特定のハードウェアに依存しないが、実装例として以下のマイコンを対象とする。それぞれのマイコンのハードウェアに依存する実装仕様は、各ハードウェア向けのμT-Kernel3.0実装仕様書を参照のこと。  

| CPUコア          | 対象マイコン        | マイコンメーカ        | マイコンボード    |
| -------------- | -------------------- | -------------- | -------------------- |
| ARM Cortex-M3  | TMPM367FDFG          | 東芝デバイス&ストレージ   | TX03 M367 IoT-Engine |
| ARM Cortex-M4  | STM32L486VG          | STマイクロエレクトロニクス | STM32L4 IoT-Engine   |
| ARM Cortex-M7  | STM32H723ZG          | STマイクロエレクトロニクス | -                    |
| ARM Cortex-M55 | STM32N657X0          | STマイクロエレクトロニクス | -                    |
| RXv2           | RX231 R5F52318ADFL   | ルネサス エレクトロニクス  | RX231 IoT-Engine     |
| RXv2           | RX65N R5F565NE       | ルネサス エレクトロニクス  | -                    |
| ARM Cortex-A9  | RZ/A2M R7S921053VCBG | ルネサス エレクトロニクス  | RZ/A2M IoT-Engine    |
...
本書に記載するμT-Kernel3.0の実装の共通仕様は特定のハードウェアに依存しないが、実装例として以下のターゲット名がある。他のハードウェアに実装する場合は、同様にターゲット名の定義を追加していく。
...
## 3.1. 起動処理
### 3.1.1. 起動処理の手順
OSの起動処理は、以下の2つの手順のいずれか、またはその両方に対応する。  

- リセットハンドラからの起動手順  
電源投入またはリセットなどにより、「3.1.2.リセットハンドラ」が実行され、OSを起動する手順である。

- カーネル起動処理からの起動手順  
リセットハンドラが別に存在しハードウェアの初期化、設定などが済んでいる状態から、OSを起動する手順である。本OSとは別にマイコンの起動処理などが存在するシステムに、本OSを組み込む際に使用することを想定している。「3.1.3.カーネル起動処理」からOSは起動される。

### 3.1.2. リセットハンドラ
リセットハンドラは、電源投入またはリセットなどにより実行される。
リセットハンドラの名称はReset_Handlerを基本とするが、ハードウェアへの実装に応じて決められる。
リセットハンドラの処理内容は、ハードウェアおよびユーザプログラムに依存するが一般的な内容を以下に記す。詳細は、各ハードウェア向けの実装仕様書を参照のこと。
...
(4) OS初期化処理の実行
カーネル起動処理を実行する。リセットハンドラに戻ることはない。

### 3.1.3. カーネル起動処理
カーネル起動処理は、本OSを起動するために実行される。
カーネル起動処理は原則としてknl_start_mtkernel関数として実装する。実装上の必要にに応じて変更しても良い。
カーネル起動処理の処理内容は、ハードウェアおよびユーザプログラムに依存するが一般的な内容を以下に記す。詳細は、各ハードウェア向けの実装仕様書を参照のこと。

(1) ハードウェアの初期化
カーネル起動処理より実行されるknl_startup_hw関数により、OSを実行する際に必要なハードウェアの初期化を行う。但し、リセットハンドラからカーネル起動処理が呼ばれた場合は、本処理は実行済みのため実行されない。
...
(5) OS拡張機能(Extension)の初期化
OSでセキュア拡張機能などの拡張機能(Extension)が有効の場合は、各拡張機能の初期化関数を実行する。

(6) OS初期化処理の実行
OSの初期化処理（「3.1.4. OS初期化処理」参照）を実行し、リセットハンドラは終了する。


### 3.1.4. OS初期化処理
OSの各機能の初期化を行う。
OS初期化処理は、以下のファイルのに関数として実装される。関数名は、コンフィギュレーションADD_PREFIX_MAIN_FUNCが0に設定されている場合は`main`、それ以外の場合は`knl_main`とする。

    /kernel/sysinit/sysinit.c

OS初期化処理では以下の処理が実行される。ただし、コンフィギュレーションなどにより実行されない処理もある。なお、OS初期化処理中は、原則OSのAPIなどの機能は利用できない。
...
初期タスクの生成情報knl_init_ctskは/include/sys/inittask.hで以下のように定義されている。ただし、セキュア機能拡張など、OS機能拡張が有効の場合は変更される場合がある。各OS機能拡張の仕様を参照のこと。
...
# 5. タスク
## 5.1. タスク属性
本実装において使用可能なタスク属性を以下に記す。
設定不可のタスク属性を指定した場合は、TA_RSATRエラーとなる。
なお、セキュア機能拡張などのOS機能拡張が有効な場合には設定可能なタスク属性が追加される場合がある。各OS機能拡張の仕様を参照のこと。
...
| CNF_TMP_STACK_SIZE | 1024   | テンポラリスタックのサイズ                                                                        |
...
(8) 静的システムメモリ領域
システムメモリ領域をOS起動時に動的に確保せず、C言語のグローバル変数として静的に確保する場合に指定する。

| 名称                 | 初期値     | 説明                                         |
| ------------------ | ------- | ------------------------------------------ |
| USE_STATIC_SYS_MEM | 0       | システムメモリ領域を静的に確保する                          |
| SYSTEM_MEM_SIZE    | 15*1024 | 静的に確保するシステムメモリ領域のサイズ<br>使用する場合は適切な値を設定すること |

(9) デバッグサポート機能  
デバッグに関連する機能を指定する。実際の対応は実装に依存する。
...
| USE_DEBUG_SYSMEMINFO  | 1   | システムメモリ情報の表示（T-Monitor出力） (0: 無 1: 有)  |

(10) キャッシュ制御
対象のハードウェアがキャッシュを有するか、およびOSの機種依存の実装により指定可能か否かが決まる。各ハードウェアの実装仕様書を参照のこと。

| 名称        | 初期値 | 説明                  |
| --------- | --- | ------------------- |
| USE_CACHE | 1   | キャッシュ対応 (0: 無 1: 有) |
...
(11) デバイスドライバ
OSにサンプルでデバイスドライバが実装されている場合、その使用を設定する。各ハードウェアの実装仕様書を参照のこと。
...
| 版数      | 日付         | 内容                                                                                                                                                                                                                                                                                                    |
| ------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.01.B0 | 2026.07.08 | μT-Kernel 3.00.08の実装に対応<br>・OS起動手順の追加<br>・OS機能拡張(セキュア機能拡張)の対応<br>・コンフィグレーションの追加                                                                                                                                                                                                                       |
